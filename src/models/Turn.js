...
Turn.init({
    ...
    validate: {
      ...
      isTime(value) {
        if (!/^\d{2}:\d{2}(:\d{2})?$/.test(value)) {
          throw new Error('El campo "hora_inicio" debe estar en formato HH:mm o HH:mm:ss');
        }
      }
    }
  },
  hora_fin: {
    ...
    validate: {
      ...
      isTime(value) {
        if (!/^\d{2}:\d{2}(:\d{2})?$/.test(value)) {
          throw new Error('El campo "hora_fin" debe estar en formato HH:mm o HH:mm:ss');
        }
      },
      isAfterHoraInicio(value) {
        if (this.hora_inicio && value <= this.hora_inicio) {
          throw new Error('El campo "hora_fin" debe ser mayor que "hora_inicio"');
        }
      }
    }
  },
