...
async function updateUserRating(userId) {
  ...
  if (totalReviews > 0) {
    const sumRatings = reviews.reduce((sum, review) => sum + review.valoracion, 0);
    const averageRating = Math.round(sumRatings / totalReviews);
    console.log('sumRatings: ', sumRatings)
    console.log('averageRating: ', averageRating)
    await User.update({ calificacion: averageRating }, { where: { id: userId } });
  } else {
...
router.post("/review", async (req, res) => {
  ...
  try {
    ...
    if (service.ClientId === reviewData.writerId && !service.calificado_x_cliente) { // Si la review la escribió el cliente y el servicio aún no fue calificado por un cliente
    ...
    } else if (!service.calificado_x_paseador) { // Sino, la review la escribió el paseador, si el servicio no fue calificado por un paseador
      await Service.update(
        {
    ...
    } else { // Si el servicio ya fue calificado por el tipo de usuario que escribió la reseña
